...
if(isset($_POST['submit'])){
  ...
  $height = $_POST['height'];                 
  $weight = $_POST['weight'];  
  $immuni= $_POST['adultimmunization'];                  
...
$query = $con->query($sql) or die ($con->error);
 ...
 if($query==1)
 {
      $sql = "INSERT INTO `tbl_adultimmunization_itr`(`family_number`,`mode_of_transaction`,
       `date_consultation`, `consultation_time`, `attending_provider`, `nature_visit`, `blood_pressure`, `temperature`,
       `respiratory_rate`, `pulse_rate`, `height`, `weight`, `immunization`, `chief_complaints`, `diagnosis`, `medical_treatment`, 
       `name_healthprovider` ) VALUES ('$fserial','$transacmode','$consuldate','$consultime','$attprovider','$natvisit',
   ...
   echo header("location: viewImmunizationrecord.php?id=".$id);
}
