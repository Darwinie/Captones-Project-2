...
if(isset($_POST['submit'])){
  ...
  $dlastdeworming = $_POST['datelastdeworming'];
  $attprovider = $_POST['attending_provider']; 
          
  
  

$sql = "UPDATE `patient_information` SET `family_number`='$fserial',`family_code`='$fcode',`barangay`='$brgy',
...
$query = $con->query($sql) or die ($con->error);
 ...
 if($query==1)
 {
      $sql = "INSERT INTO `tbl_pediadeworming_itr`(`family_number`,`mode_of_transaction`,
       ...
       `temperature`, `date_lastdeworming`, `attending_provider`) VALUES ('$fserial','$transacmode',
       '$ddeworming','$dtime',' $natvisit',
       '$weight',' $height',' $temp','$dlastdeworming','$attprovider')";
        $query = $con->query($sql) or die ($con->error);
             if($query==1){
               echo "Saved";
             }
   ...
   echo header("location: viewbrgy_deworming.php?id=".$id);
}
